...
/**
 * The CustomMapKit class extends the JXMapViewer class and seeks to extend the
 * latter's functionality by handling Waypoint management as well as adding zoom
 * buttons and a minimap.
 * 
 * @author Chad
...
public class CustomMapKit extends JXMapViewer {
	...
	/**
	 * This is the only constructor and takes no arguments. It handles all the
	 * set up of the map kit on its own.
	 * 
	 * @author Chad
	 * 
	 */
	public CustomMapKit() {
		...
		setRestrictOutsidePanning(true); // make sure you cant pan too far up or
											// down
		...
		setZoom(15); // start at zoom level 10
		setAddressLocation(new GeoPosition(43.701637, -79.392929)); // start
																	// with the
																	// map
																	// centered
																	// on
	...
	/**
	 * The setupMouseEvents() method assigns the mouseClicked and mouseMoved
	 * events. When the user right-clicks on the map an appropriate popup menu
	 * is opened depending on whether or not the user clicked on an existing
	 * waypoint. When the mouse is moved a label is displayed if near a waypoint
	 * that has an address.
	 * 
	 * @author Chad
	 */
	private void setupMouseEvents() {
		addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (e.getButton() == MouseEvent.BUTTON3) { // user right-clicked
					Point2D topcorner = getViewportBounds().getLocation(); // find
																			// the
																			// point
																			// to
																			// offset
					...
					lastClicked = new WaypointExt(getTileFactory().pixelToGeo(
							new Point2D.Double(topcorner.getX() + e.getX(), topcorner.getY() + e.getY()), getZoom()));
		...
		addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				if (e.getButton() == MouseEvent.BUTTON1) { // user left-clicked
					Point2D topcorner = getViewportBounds().getLocation(); // find
																			// the
																			// point
																			// to
					...
					if (coder.reverseLookup(lastClicked.getPosition().getLatitude(),
							lastClicked.getPosition().getLongitude())) {
		...
		addMouseMotionListener(new MouseMotionListener() {
			...
			public void mouseMoved(MouseEvent e) {
				Rectangle view = getViewportBounds(); // find the point to
														// offset by
				// calculate the actual on the map
				WaypointExt position = new WaypointExt(getTileFactory()
						.pixelToGeo(new Point2D.Double(view.x + e.getX(), view.y + e.getY()), getZoom()));
	 ...
	 * that show when the user right-clicks on the map and allow for addition of
	 * a waypoint at that location or removal of an already existing waypoint
	 * 
	 * @author Chad
	 */
	...
	private void setupZoomButtons() {
		...
		plus.setIcon(new ImageIcon("src/resources/plus.png")); // icon grabbed
																// from JXMapKit
		...
		minus.setIcon(new ImageIcon("src/resources/minus.png")); // icon grabbed
																	// from
																	// JXMapKit
		...
		jsZoom.addChangeListener(new ChangeListener() {
			...
			public void stateChanged(ChangeEvent e) {
				setZoom(((JSlider) e.getSource()).getValue()); // change the
																// zoom level
			}
		});
	...
	private void setupMiniMap() {
		miniMap.setBorder(BorderFactory.createEtchedBorder(EtchedBorder.RAISED)); // add
																					// a
																					// border
																					// so
																					// you
		...
		Painter<JXMapViewer> viewportBox = new Painter<JXMapViewer>() {
			public void paint(Graphics2D g, JXMapViewer map, int width, int height) {
				...
				Point2D se = new Point2D.Double(nw.getX() + viewport.getWidth(), nw.getY() + viewport.getHeight());

				// convert the points to geographic locations and then back to
				// the point as it corresponds to the minimap
				// Point2D nw =
				// map.convertGeoPositionToPoint(convertPointToGeoPosition(nw));
				// // these dont work
				// Point2D se =
				// map.convertGeoPositionToPoint(convertPointToGeoPosition(se));
				// // for some reason
				...
				Rectangle2D.Double shp = new Rectangle2D.Double(nw.getX(), nw.getY(), // x,
																						// y
	...
	/**
	 * The TileFactory code was taken from the website and without it the map
	 * wouldn't load http://javafx.com/samples/ExercisingSwing/index.html
	 */
	private void setupMapSource() {
		final int max = 17;
		TileFactoryInfo info = new TileFactoryInfo(1, max - 2, max, 256, true, true, // tile
																						// size
																						// is
																						// 256
																						// and
	...
	/**
	 * The setZoom(int) method overrides JXMapViewers setZoom(int) and adds
	 * functionality for continuity with the minimap and with the zoom slider on
	 * the side.
	 * 
	 * @description When setting a new zoom level 0 is zoomed all the way in and
	 *              15 is all the way out.
	 * @author Chad
	 * @param zoom
	 *            the integer value that you want to set the new zoom to.
	 */
	@Override
	public void setZoom(int zoom) {
		super.setZoom(zoom);

		miniMap.setZoom(zoom + 3); // keep the minimap 3 levels zoomed out from
									// the main map if possible
		jsZoom.setValue(zoom); // keep the slider up to date with everything
								// else
	}
	 ...
	 * 
	 * @description The method checks for duplicate Waypoints and rejects any
	 *              duplicates
	 * @author Chad
	 * @param wp
	 *            is the new waypoint to draw
	 * @return returns true if the waypoint was successfully added, false
	 *         otherwise
	 */
	...
	/**
	 * The removeWaypoint() method attempts to remove the passed WaypointExt
	 * object.
	 * 
	 * @author Chad
	 * @param wp
	 *            is the waypoint to try and remove
	 * @return returns true if the waypoint was successfully removed, false
	 *         otherwise
	 */
	public boolean removeWaypoint(WaypointExt wp) {
		 ...
		 * System.out.println(wps.iterator().next().equals(wp)); //Dont know why
		 * the above won't work
		 */

		// I have to jump through some hoops to try and remove a Waypoint
		// because
		// the built-in Set object won't call my overridden Waypoint.equals(o)
		// method
	...
	/**
	 * The WaypointExt(Point2D) method determines if the given Point2D is within
	 * 6 pixels(?) of an already existing Waypoint,
	 * 
	 * @author Chad
	 * @param pt
	 *            is the Point2D that you want to search for
	 * @return returns the closest Waypoint that it can find, null if none are
	 *         found
	 */
	public WaypointExt waypointNear(WaypointExt check) {
		...
		Object[] wparray = wps.toArray(); // it doesnt like getting cast to a
											// Waypoint[]
											// so I have to cast each item when
											// I use it
		...
		for (int i = 0; i < wparray.length; i++) {
			...
			if (wppt.distance(checkpt) < longestDistance) {
				w = (WaypointExt) wparray[i]; // if the waypoint is the closest
												// record it
			}
		}
	...
	/**
	 * The setAddressLocation method passes the location onto the super class
	 * method and sets the minimap to the same location.
	 * 
	 * @author Chad
	 * @param addressLocation
	 *            the location that you want to send the map to
	 */
	@Override
	...
	/**
	 * The setCenterPosition method passes the location onto the super class
	 * method and sets the minimap to the same location.
	 * 
	 * @author Chad
	 * @param geoPosition
	...
	/**
	 * The setCenter method passes the location onto the super class method and
	 * sets the minimap to the same location after taking into account zoom.
	 * 
	 * @author Chad
	 * @param pt
	 *            the location that you want to send the map to
	...
	public void setCenter(Point2D pt) {
		...
		miniMap.setCenter(
				miniMap.getTileFactory().geoToPixel(getTileFactory().pixelToGeo(pt, getZoom()), miniMap.getZoom()));
	}

	/**
	 * getCoder() returns a reference to the Geocode instance object in the
	 * mapkit.
	 * 
	...
	/**
	 * The setMiniMapVisible method turns the minimap on or off depending on the
	 * value of the passed boolean.
	 * 
	 * @param visible
	 */
	...
	public void setMiniMapSize(int width, int height) {
		miniMap.setPreferredSize(new Dimension(width, height));
	}
	...
	public void setZoomSliderSize(int width, int height) {
		jsZoom.setPreferredSize(new Dimension(width, height));
	}
}